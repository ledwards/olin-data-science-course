...
n <- 10 # Choose a sample size
df_q1 <- tibble(z = rnorm(n)) # Generate the data
glimpse(df_q1)
```
...
You now have an estimate of $\pi$, but how trustworthy is that estimate? In `e-stat06-clt` we discussed *confience intervals* as a means to quantify the uncertainty in an estimate. Now you'll apply that knowledge to assess your $\pi$ estimate.
